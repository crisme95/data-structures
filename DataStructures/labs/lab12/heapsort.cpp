...
void heapify(std::vector<T> &heap)
{
  ...
  int level = last_non_leaf(heap.size());
  // std::cout << "index = " << level << " value = " << heap[level] << std::endl;

  while (level != -1)
  {
    int lchild = left_child(level);
    int rchild = right_child(level);

    if (lchild < (int)heap.size())
    {
      percolate_down(heap, lchild);
    }

    if (rchild < (int)heap.size())
    {
      percolate_down(heap, rchild);
    }
    percolate_down(heap, level);
    level -= 1;
  }
}
